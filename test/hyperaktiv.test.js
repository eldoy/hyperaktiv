...
test('asynchronous computation', async () => {
  ...
  const delayedAddOne = computed(
    ({
      computeAsync
    }) => delay(200).then(() => computeAsync(addOne)), {
      autoRun: false
    }
  )
  await delayedAddOne()
...
test('concurrent asynchronous computations', async () => {
  ...
  const plus = prop => computed(async ({
    computeAsync
  }) => {
    await delay(200)
...
test('not observe ignored properties (props & ignore: array<string>)', () => {
  ...
  const observeA = observe(object, {
    props: ['a']
  })
  const observeB = observe(object, {
    ignore: ['a', 'sum']
  })
...
test('bind methods to the observed class', () => {
  ...
  const observer = observe(new TestClass(), {
    bind: true
  })
